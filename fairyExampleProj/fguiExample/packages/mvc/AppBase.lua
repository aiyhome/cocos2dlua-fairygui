...
function AppBase:run(initSceneName, extData)
    initSceneName = initSceneName or self.configs_.defaultSceneName
    return self:enterScene(initSceneName,nil,nil,nil, extData)
end

function AppBase:enterScene(sceneName, transition, time, more, extData)
    local view = self:createView(sceneName, extData)
    view:showWithScene(transition, time, more)
    return view
end

function AppBase:createView(name, extData)
    for _, root in ipairs(self.configs_.viewsRoot) do
        ...
        if status and (t == "table" or t == "userdata") then
            return view:create(self, name, extData)
        end
    end
